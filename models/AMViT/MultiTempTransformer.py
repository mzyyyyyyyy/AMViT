...
class Local_Relational_Block(nn.Module):
    ...
    def __init__(self, model_config, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
    ...
    def forward(self, x):
        x_normal = x[:, self.cls_token_num:, :]
        x_normal = self.linear1(x_normal)
        x_normal = x_normal.transpose(1, 2)
        x_normal = self.TC(x_normal)
        x_normal = x_normal.transpose(1, 2)
        x_normal = self.act(x_normal)
        x_normal = self.drop(x_normal)
        x_normal = self.linear2(x_normal)
        x_normal = self.drop(x_normal)
        x = torch.cat((x[:, :self.cls_token_num, :], x_normal), dim=1)
        return x
...
class Temporal_Merging_Block(nn.Module):
    ...
    def __init__(self, model_config, kernel_size=3, stride=1, in_chans=128, embed_dim=128):# 这些参数得在 config 文件中重新定义。
        super().__init__()
    ...
    def forward(self, x):
        ...
        x_normal = x_normal.transpose(1, 2)
        x = torch.cat((x[:, :, :self.cls_token_num], x_normal), dim=2)
...
class GLRBlock(nn.Module):
    ...
    def __init__(self, model_config, dim, num_heads, mlp_ratio=4., drop=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        self.Local_Relational_Block = Local_Relational_Block(model_config, in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class MultiTempTransformer(nn.Module):
    def __init__(self, model_config, in_feat_dim=128, embed_dims=[128, 128, 128, 128],
                 num_head=8, mlp_ratio=8, norm_layer=nn.LayerNorm,
                 num_block=3): # 这些参数得在 config 文件中重新定义一下。
        super().__init__()
        ...
        self.block1 = nn.ModuleList([GLRBlock(
            model_config, dim=embed_dims[0], num_heads=num_head, mlp_ratio=mlp_ratio,norm_layer=norm_layer)
        ...
        self.block2 = nn.ModuleList([GLRBlock(
            model_config, dim=embed_dims[1], num_heads=num_head, mlp_ratio=mlp_ratio,norm_layer=norm_layer)
            for i in range(num_block)])
        ...
        self.block3 = nn.ModuleList([GLRBlock(
            model_config, dim=embed_dims[2], num_heads=num_head, mlp_ratio=mlp_ratio,norm_layer=norm_layer)
            for i in range(num_block)])
        ...
        self.block4 = nn.ModuleList([GLRBlock(
            model_config, dim=embed_dims[3], num_heads=num_head, mlp_ratio=mlp_ratio,norm_layer=norm_layer)
            for i in range(num_block)])
    ...
    def forward(self, x):
        ...
        # input_shape = (b, c, n) transpose 之后的 shape
        # output_shape = (b, n, c)
